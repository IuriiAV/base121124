...
public class KommerzATM extends ATM {

    public KommerzATM(int atmBalance) {
        super("Kommerz", atmBalance, "USD");
    }
    ...
    public boolean withdraw(CreditCard card, int amount) {
        if (atmBalance >= amount && card.getBalance() >= amount) {
            ...
            System.out.println("Успешное снятие денег через банкомат Kommerz");
            return true;
        } else {
