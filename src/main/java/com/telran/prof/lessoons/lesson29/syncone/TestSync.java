...
public class TestSync implements Runnable {
    ...
    Если тот в момент, когда поток хочет выполнить синхронизированный метод,
    но мьютекс этого объекта занят(то другой объект выполняет его), то поток
    который хотел с ним работать, переходит в блокированное состояние до тех пор
    пока мьютекс этого объекта не освободится и он не сможет продолжить работу с этим
    методом
     */
...
//    }
    ...
    public synchronized void run() { // synchronized in method === synchronized (this)
        ...
        for (int i = 0; i < 100; i++) {
            //synchronized (this) { // 1.read 2.inc 3.write
                SyncApp.counter++;
            //}
